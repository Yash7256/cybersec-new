...
async def load_scan_with_results(
    scan_id: UUID,
    db: AsyncSession,
    current_user: User,
) -> tuple[Scan, list[ScanResult]]:
    """Load a scan and its results, enforcing ownership."""
...
async def export_json(
    ...
    scan, results = await load_scan_with_results(scan_id, db, current_user)
    ...
    report = Report(scan_id=scan.id, user_id=current_user.id, format="json")
    db.add(report)
    await db.commit()
...
async def export_csv(
    ...
    report = Report(scan_id=scan.id, user_id=current_user.id, format="csv")
    db.add(report)
    await db.commit()
...
async def export_pdf(
    ...
    title_style = ParagraphStyle("TitleStyle", parent=styles["Heading1"], fontName="Helvetica-Bold", fontSize=24, textColor=PURPLE, spaceAfter=6)
    subtitle_style = ParagraphStyle("SubtitleStyle", parent=styles["Normal"], fontName="Helvetica", fontSize=14, textColor=WHITE, spaceAfter=12)
    section_style = ParagraphStyle("SectionStyle", parent=styles["Heading2"], fontName="Helvetica-Bold", fontSize=16, textColor=PURPLE, spaceBefore=18, spaceAfter=12)
    normal_style = ParagraphStyle("NormalStyle", parent=styles["Normal"], fontName="Helvetica", fontSize=10, spaceAfter=6)
    ...
    meta_data = [
        ["Target:", scan.target],
        ["Scan Type:", scan.scan_type or "None"],
        ["Status:", scan.status or "None"],
        ["Date:", scan.created_at.isoformat() if scan.created_at else "None"],
        ["Duration:", duration_str],
        ["Port Range:", scan.port_range or "None"],
    ]
    ...
    overall_risk = "CRITICAL" if has_critical else ("HIGH" if has_high else ("MEDIUM" if has_medium else "LOW"))
    ...
    exec_data = [["Open Ports", str(len(open_ports))], ["Total Scanned", str(len(results))], ["CVEs Found", str(total_cves)], ["Risk Level", overall_risk]]
    exec_table = Table(exec_data, colWidths=[150, 150])
    exec_table.setStyle(TableStyle([
        ("BACKGROUND", (0, 0), (1, 0), port_color), ("TEXTCOLOR", (0, 0), (1, 0), WHITE),
        ("BACKGROUND", (0, 1), (1, 1), GRAY), ("TEXTCOLOR", (0, 1), (1, 1), WHITE),
        ("BACKGROUND", (0, 2), (1, 2), cve_color), ("TEXTCOLOR", (0, 2), (1, 2), WHITE),
        ("BACKGROUND", (0, 3), (1, 3), risk_color), ("TEXTCOLOR", (0, 3), (1, 3), WHITE),
        ("ALIGN", (0, 0), (-1, -1), "CENTER"), ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
        ("FONTNAME", (0, 0), (-1, -1), "Helvetica-Bold"),
        ("GRID", (0, 0), (-1, -1), 1, WHITE), ("PADDING", (0, 0), (-1, -1), 8),
    ]))
    ...
    def get_risk_color(cves):
        ...
        for cve in (cves or []):
            ...
            elif sev == "MEDIUM" and highest != "HIGH": highest = "MEDIUM"
        if highest == "HIGH": return ORANGE
        if highest == "MEDIUM": return YELLOW
        return GREEN if not cves else GRAY
    ...
    if not open_ports:
        ...
        port_style = TableStyle([
            ...
            ("BOTTOMPADDING", (0, 0), (-1, 0), 8), ("GRID", (0, 0), (-1, -1), 0.5, GRAY),
        ])
        for i, r in enumerate(open_ports, start=1):
            r_cves = r.cves if isinstance(r.cves, list) else []
            r_color = get_risk_color(r_cves)
            p_risk = {RED: "CRITICAL", ORANGE: "HIGH", YELLOW: "MEDIUM", GREEN: "LOW"}.get(r_color, "INFO")
            port_data.append([str(r.port), r.protocol or "", r.service or "", r.version or "", p_risk])
    ...
    if not all_cves_list:
        ...
        for i, cve in enumerate(all_cves_list, start=1):
            sev = cve.get("severity", "").upper()
            c_color = {RED: RED, "CRITICAL": RED, "HIGH": ORANGE, "MEDIUM": YELLOW, "LOW": GREEN}.get(sev, GRAY)
            desc = cve.get("description", "")
            if len(desc) > 100: desc = desc[:100] + "..."
            cve_data.append([str(cve.get("port", "")), cve.get("id", ""), str(cve.get("cvss_score", "")), sev, Paragraph(desc, normal_style)])
    ...
    report = Report(scan_id=scan.id, user_id=current_user.id, format="pdf")
    db.add(report)
    await db.commit()
